...
def on_version(device_id, hv, sv):
    try:
        ...
        mock_response_data = {
            'data': {
                'data': {
                    'sv': sv,
                    'hv': hv,
                    'url': 'http://example.com/update-file.zip',
        ...
        v['key'] = m.hexdigest()
        v['size'] = str(r['size'])
        push_str = "UPUP" + str(json.dumps(v)).replace("https://", "http").replace(",", ", ").replace(":",": ").\
